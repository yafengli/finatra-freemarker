...
object BeetlConfigurationFactory {
  def apply(templateDir: String): BeetlConfigurationFactory = {
    BeetlConfigurationFactory(new GroupTemplate(new ClasspathResourceLoader(templateDir + "/", Charsets.UTF_8.name()), Configuration.defaultConfiguration))
  }

  def apply(templateDir: File): BeetlConfigurationFactory = {
    BeetlConfigurationFactory(new GroupTemplate(new FileResourceLoader(templateDir.getAbsolutePath, Charsets.UTF_8.name()), Configuration.defaultConfiguration))
  }
}